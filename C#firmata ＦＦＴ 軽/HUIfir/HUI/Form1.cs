        ...
        private void plota0(string data)
        {
            if (data != null)
            {
              ...
              a0Box.AppendText(Volt.ToString("#0.00") + Environment.NewLine);
                ...
                int VoltG = Convert.ToInt32(Volt * 10);
                Graphics GR = pictureBox1.CreateGraphics();
                Pen p = new Pen(Color.Black);
                data00 *= -1;
                GR.DrawLine(p, time, prevolte, time + 1, VoltG);
                GR.ScaleTransform(0.3f, 0.015f);
                time++;
                prevolte = VoltG;
                if (time > 400)
                {
        ...
        private void plota1(string data)
        {
            if (data != null)
            {
               ...
               a1Box.AppendText(Volt.ToString("#0.00") + Environment.NewLine);
                ...
                Pen p = new Pen(Color.Black);
                data00 *= -1;
                GR.DrawLine(p, time1, prevolte1, time1 + 1, VoltG);
                GR.ScaleTransform(0.3f, 0.015f);
                time1++;
                prevolte1 = VoltG;
                if (time1 > 400)
        ...
        private void plota2(string data)
        {
            if (data != null)
            {
               ...
               a2Box.AppendText(Volt.ToString("#0.00") + Environment.NewLine);
                ...
                Pen p = new Pen(Color.Black);
                data00 *= -1;
                GR.DrawLine(p, time2, prevolte2, time2 + 1, VoltG);
                GR.ScaleTransform(0.3f, 0.015f);
                time2++;
                prevolte2 = VoltG;
                if (time2 > 400)
